...
function profileCode(match) {
  ...
  if (match.match_kind === 'WORLD_CUP') return match.stage_type === 'GROUP' ? 'CONTINENTAL_A' : 'WORLD_CUP_ELITE';
...
function evaluateCandidate(stadium, profile) {
  ...
  const strictTechnology = ['WORLD_FINAL','WORLD_CUP_ELITE'].includes(profile.code);
  const features = {
  ...
  for (const [requiredField, stadiumField] of Object.entries(features)) {
    if (Boolean(profile[requiredField]) && !Boolean(stadium[stadiumField])) {
      const safety = stadiumField === 'has_medical_center' || stadiumField === 'has_backup_power';
      add(stadiumField, true, false, safety || strictTechnology);
    }
  }
  return {
