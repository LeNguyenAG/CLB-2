...
async function autoAssign() {
  ...
  try {
    ...
    uiStore.notify(assigned ? `Đã đề xuất linh vật riêng cho ${assigned} CLB.` : 'Tất cả CLB đã có linh vật hoặc kho 12 mẫu đã đủ.')
    await load()
  } catch (error) { uiStore.notify(error.message, 'error') }
  finally { loading.value = false }
}
...
<template>
  <div>
    <PageHeader eyebrow="Club Mascot Studio" title="Linh vật câu lạc bộ" description="Chọn bản sắc riêng để xuất hiện trên hồ sơ CLB và toàn bộ nhánh đấu loại trực tiếp.">
