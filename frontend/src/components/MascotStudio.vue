...
import { clubMascots, mascotCategories, mascotFor, suggestMascot } from '../data/clubMascots'
...
const assignments = ref([]), selectedKey = ref(''), activeCategory = ref('ALL'), loading = ref(true), busy = ref(false)
...
const filteredMascots = computed(() => activeCategory.value === 'ALL' ? clubMascots : clubMascots.filter((mascot) => mascot.category === activeCategory.value))
const categoryCount = (category) => category === 'ALL' ? clubMascots.length : clubMascots.filter((mascot) => mascot.category === category).length
...
<template>
  <section class="mascot-studio">
    ...
    <template v-else><div class="mascot-categories"><button v-for="category in mascotCategories" :key="category.key" type="button" :class="{active:activeCategory===category.key}" @click="activeCategory=category.key">{{ category.name }} <span>{{ categoryCount(category.key) }}</span></button></div>
    <div class="mascot-grid">
      <button v-for="mascot in filteredMascots" :key="mascot.key" type="button" :class="{ selected: selectedKey === mascot.key, occupied: occupied.has(mascot.key) }" :disabled="current.mascot_locked && !admin" @click="selectedKey=mascot.key">
