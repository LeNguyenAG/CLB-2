...
router.get('/public/clubs', async (req, res) => {
  ...
  const rows = await query(
    `SELECT c.id, c.code, c.name, c.short_name,
            CASE WHEN ms.setting_value IS NOT NULL THEN CONCAT('/mascots/', SUBSTRING_INDEX(ms.setting_value, '|', 1), '.webp') ELSE c.logo_url END AS logo_url,
            c.logo_url AS official_logo_url,
...
router.get('/public/clubs/:id', async (req, res) => {
  ...
  const club = await first(
    `SELECT c.id, c.code, c.name, c.short_name,
            CASE WHEN ms.setting_value IS NOT NULL THEN CONCAT('/mascots/', SUBSTRING_INDEX(ms.setting_value, '|', 1), '.webp') ELSE c.logo_url END AS logo_url,
            c.logo_url AS official_logo_url, c.registration_status,
            SUBSTRING_INDEX(ms.setting_value, '|', 1) AS mascot_key,
