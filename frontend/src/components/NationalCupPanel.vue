...
const finalizeStatus = computed(() => {
  if (data.value?.profile?.tournament_finalized_at)
    return { ready: true, label: "Giải đã được chốt và trao thưởng đầy đủ." };
  const remaining = Math.max(0, 32 - completedMatches.value);
  if (remaining)
    return { ready: false, label: `Còn ${remaining} trận chưa hoàn tất nên chưa thể chốt giải.` };
  return { ready: true, label: "Đã đủ 32/32 kết quả — sẵn sàng chốt và trao giải." };
});
  ...
  >
    ...
    <section v-else-if="active === 'bracket'" class="bracket-stage">
      ...
      <KnockoutBracket
        ...
        :match-links="data.matchLinks || []"
        :historical-achievements="data.historicalAchievements || []"
        :previous-podium="data.previousPodium || []"
        :admin="admin"
        @result="openResult"
      />
      <div v-if="false" class="bracket-arena">
    ...
    <section v-else class="rewards-grid">
      <article v-if="admin" class="glass card finalize-card">
        ...
        <div>
          ...
          <span class="finalize-status" :class="{ ready: finalizeStatus.ready }">
            {{ finalizeStatus.label }}
          </span>
        </div>
        <button
          class="btn btn-success"
          :disabled="busy || completedMatches !== 32 || data.profile.tournament_finalized_at"
          @click="finalizeTournament"
        >
          <CheckCircle2 :size="16" />{{ data.profile.tournament_finalized_at ? "Đã chốt giải" : "Chốt & trao giải" }}
        </button>
        <button
...
.finalize-status {
  ...
  padding: 6px 9px;
  border-radius: 9px;
  background: rgba(255, 176, 68, 0.09);
  color: #ffc46b;
  font-size: 10px;
  font-weight: 850;
}
.finalize-status.ready {
  background: rgba(69, 212, 151, 0.1);
  color: #6ee3ab;
}
.result-form {
  display: grid;
